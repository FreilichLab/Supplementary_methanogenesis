...
met_size_log_factor = config['met_size_log_factor']

met_size_over_all_factor = config['met_size_over_all_factor']

bm_factor = config['bm_factor']
...
# dict of bacterium biomass representative value in time.

bac_BM_dict = bac_bm_calc.to_dict() 

bac_BM_dict2 = {} # for better appearance 

for k in bac_BM_dict.keys():
    ...
    bac_BM_dict2[nam] = bac_BM_dict[k]
...
for k in dic_namform.keys():

    namc = dic_namform[k][0]

    namc = namc.split('_')

    namc = namc[0]

    dic_namform2[k]=[namc,dic_namform[k][1]]
...
def met_size(row):
   ...
   if row['vv'] > 0:
      ...
      cc = (np.log2(row['vv']*met_size_log_factor))*met_size_over_all_factor 
...
# dict of compounds representative value in time.

met_calc_dict = met_calc.to_dict()
...
for file_name in lines:
    ...
    for col in mod1.columns:
        ...
        if 'cpd11416' not in col:
            ...
            if col2 in dic_namform2.keys():

                formula = dic_namform2[col2][1]

                if formula == 'H' or (formula[0] == 'C' and formula[1].islower() == False):
...
for k in dic_EX_files.keys():
    ...
    if day in df_flux.index:
        ...
        df_EX_secretion = df_flux_day_[(df_flux_day_["flux"] > 0.00000000000015)]

        df_EX_uptake = df_flux_day_[(df_flux_day_['flux'] < -0.00000000000015)]
...
for tup in uptakes:
    ...
    newbac = tup[1].split('_')

    newbac = newbac[0:2]

    newbac = ' '.join(newbac)

    newtup = (tup[0],newbac,tup[2])

    uptakes2.append(newtup)

secretions2 = []

for tup in secretions:

    newbac = tup[0].split('_')

    newbac = newbac[0:2]

    newbac = ' '.join(newbac)

    newtup = (newbac,tup[1],tup[2])

    secretions2.append(newtup)

secretionsN = [] 

for tup in secretions2:

    id = tup[1].replace('EX_','')

    name = dic_namform2[id][0]
...
uptake_of_interest = config['uptake_of_interest']

secretions_of_interest = config['secretions_of_interest']
...
# define edges width, define vertical groups (layers) of bacteria

#and metabolites, and define color and size of nodes.

edges_factor_to_devide = config['edges_factor_to_devide']
...
for node in G.nodes:
    ...
    if node in bac_A:
        ...
        node_sizes.append(bac_BM_dict2[node])
...
plt.savefig(output_dir + plot_filename, dpi=600) 
